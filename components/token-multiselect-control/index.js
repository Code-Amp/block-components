...
export class TokenMultiSelectControlBase extends Component {
	...
	onKeyDown( event ) {
		...
		switch ( event.keyCode ) {
			...
			case ENTER:
				...
				if (
					this.props.preventDefaultOnEnter &&
					this.props.preventDefaultOnEnter === true
				) {
					preventDefault = true;
				}
				break;
	...
	getMatchingSuggestions(
		...
		// quick and dirty - remove duplicates from suggestions
		// TODO - pass in id + label to suggestions list for generating keys based on IDs
		suggestions = suggestions.filter(
			( v, i ) => ! suggestions.includes( v, i + 1 )
		);
